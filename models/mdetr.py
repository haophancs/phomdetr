...
class QACriterionClevr(nn.Module):
    ...
    def forward(self, output, answers, return_ga_pa=False, answer_decoder=None):
        ...
        attr_norm = is_attr.sum() if is_attr.any() else 1.0
        loss["loss_answer_attr"] = (
            F.cross_entropy(output["pred_answer_attr"].cpu(), answers["answer_attr"].cpu(), reduction="none")
            .masked_fill(~is_attr, 0)
            .sum()
            / attr_norm
        )
        ...
        loss['report_weighted_precision'] = report['weighted avg']['precision']
        loss['report_weighted_recall'] = report['weighted avg']['recall']
        loss['report_macro_f1'] = report['macro avg']['f1-score']
